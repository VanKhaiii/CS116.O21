...
def run_train(public_dir, model_dir):
    ...
    # hc_cols = [9, 11, 18, 28, 30, 33, 35, 38, 39, 41, 44, 45, 47, 49, 50, 51, 52, 53, 54, 56, 58, 77, 82, 83, 84, 86, 87, 88, 94, 97]
    # X_train = np.delete(X_train, hc_cols, axis=1)
    # Instantiates the logistic regression model.
    # model = LogisticRegression()
    # model = RandomForestClassifier(max_depth=13,n_estimators=26,random_state=52).fit(X_train,y_train)
    
    svc_subpipeline = Pipeline(steps=[
    ('clf', SVC(gamma='auto'))
    ])
    ...
    param_grid=[
                {'clf__C': x,'clf__kernel': ['rbf']} ,
                {'clf__C': x,'clf__kernel': ['poly'],'clf__degree': z}
               ]
    
    grid_search = GridSearchCV(svc_subpipeline, param_grid, n_jobs=-1, scoring='f1', verbose=True, refit=True)
    ...
    # rfecv = RFECV(estimator = model, step = 1, cv = 5, scoring = 'f1', n_jobs = -1).fit(X_train,y_train)
...
def run_predict(model_dir, public_dir, output_path):
    ...
    # hc_cols = [9, 11, 18, 28, 30, 33, 35, 38, 39, 41, 44, 45, 47, 49, 50, 51, 52, 53, 54, 56, 58, 77, 82, 83, 84, 86, 87, 88, 94, 97]
    # X_test = np.delete(X_test, hc_cols, axis=1)
